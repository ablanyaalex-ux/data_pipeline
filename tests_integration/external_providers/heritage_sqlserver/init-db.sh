...
echo "Creating database [Static Dimensions] if missing..."
/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P "$SA_PASSWORD" -C -d master -Q \
"IF DB_ID(N'Static Dimensions') IS NULL CREATE DATABASE [Static Dimensions];"

echo "Waiting for [Static Dimensions] to be ONLINE..."
/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P "$SA_PASSWORD" -C -d master -Q \
"DECLARE @i int=0;
 WHILE (@i < 120 AND (SELECT state_desc FROM sys.databases WHERE name = N'Static Dimensions') <> 'ONLINE')
 BEGIN
   WAITFOR DELAY '00:00:01';
   SET @i += 1;
 END;
 SELECT name, state_desc FROM sys.databases WHERE name = N'Static Dimensions';"

echo "Waiting additional time for database to be fully accessible..."
sleep 2

echo "Running schema script against [Static Dimensions]..."
...
echo "Running remaining scripts against [Static Dimensions]..."
